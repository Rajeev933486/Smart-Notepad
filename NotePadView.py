...
class Notepad:
    ...
    def find_func(self, event=None):
        self.find_dialouge=tk.Toplevel()
        self.find_dialouge.geometry('450x250+500+200')
        self.find_dialouge.title('Find')
        self.find_dialouge.resizable(0,0)

        ## frame
        self.find_frame=ttk.LabelFrame(self.find_dialouge,text='Find/Replace')
    ...
    def secure_file(self, event=None):
        if self.db_status:
            self.file_dialouge=tk.Toplevel()
            self.file_dialouge.geometry('450x400+500+200')
            self.file_dialouge.title('Secure Files')
            self.file_dialouge.resizable(0,0)

            ## frame
            self.file_frame=ttk.LabelFrame(self.file_dialouge,text='Add Secure Files')
            ...
            # self.file.replace_button=ttk.Button(self.file_frame,text='Replace',command=self.replace)
            ...
            ## scrollbar list
            scrollbar=ttk.Scrollbar(self.file_dialouge,orient='vertical')
            scrollbar.pack(side='right', fill='both')
            self.fileList=tk.Listbox(self.file_dialouge)
            ...
            self.file_dialouge.mainloop()
        else:
            messagebox.showerror("Database Error", "Please Connect with the Database")

    def open_secure_file(self):
        try:
            self.set_file_index_tuple=self.fileList.curselection()
            if len(self.set_file_index_tuple)==0:
                messagebox.showerror("Error!","Please select a file first")
            else:
                self.full_file_name=self.fileList.get(self.set_file_index_tuple[0])
    ...
    def remove_secure_file(self):
        try:
            ...
            if len(self.set_file_index_tuple)!=0:
                ...
                self.fileList.delete(self.set_file_index_tuple[0])
